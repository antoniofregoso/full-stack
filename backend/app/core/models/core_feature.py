...
class CoreFeature(CoreAudit, SQLModel, table=True):
    __tablename__ = "core_features"
    __table_args__ = (
        UniqueConstraint("entity_id", "code", name="uq_core_features_entity_code"),
    )
    ...
    icon: dict[str, str] = Field(default_factory=dict, sa_type=JSONB)
    description: Optional[dict[str, str]] = Field(default=None, sa_type=JSONB)
    active: bool = Field(default=True)
    sequence: int = Field(default=10)
    entity: "CoreEntity" = Relationship(back_populates="features")
